...
interface NavLinksProps { isMobile?: boolean; onClickLink?: () => void; }
...
const NavLinks = memo(({ isMobile = false, onClickLink }: NavLinksProps) => {
  ...
  const cvFile = i18n.language === 'es' ? '/resume/CV_Andy_Álvarez.pdf' : '/resume/Resume_Andy_Álvarez.pdf';
  ...
  return (
    <>
      ...
      <div className={`cursor-pointer ${isMobile ? 'mt-8' : 'ml-4'}`}><LanguageSelector /></div>
      <Button text={t('header.resume')} onClickEvent={() => handleDownload(cvFile, cvName)} size={isMobile ? 'lg' : 'md'} className={`${isMobile ? 'mt-6' : 'ml-6'}`}>
        <MdDownload className={`${isMobile ? 'text-2xl' : 'text-lg'} align-middle -mb-0.5 text-white`} />
      </Button>
    </>
  );
});
